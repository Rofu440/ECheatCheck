...
public final class AutoMessageTask {
    ...
    public void start() {
        ...
        if (!cheatCheck.getConfig().isAutoMessagesEnabled()) return;
    ...
    private void run() {
        ...
        try {
            ...
            logger.severe("Ошибка в задаче автосообщений для игрока " +
    ...
    public void setIntervalSeconds(int seconds) {
        if (seconds <= 0) throw new IllegalArgumentException("Интервал должен быть положительным");
